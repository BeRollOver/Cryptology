...
namespace Cryptology
{
    class Stribog
    {
        // Матрица A для L-преобразования
        private ulong[] A = {
        ...
        // Таблица подстановок п для S-преобразования
        private byte[] Sbox ={
        ...
        // Таблица перестановок т для P-преобразования
        private byte[] Tau ={
        ...
        // Набор постоянных значений для функции KeySchedule
        private byte[][] C = {
        new byte[64]{
        ...
        // Присвоение начальных начений внутренних переменных
        ...
        {
            // Присваиваем начальные значений внутренних переменных
            ...
            while (len >= 512)
            {
                ...
                // tempMes - последние 512 бит сообщения M
                ...
                // Обрезаем последние 512 бит сообщения
                len -= 512;
            }
            // Оставляем сообщение длиной меньше 512 бит
            byte[] message1 = new byte[message.Length - inc * 64];
            Array.Copy(message, 0, message1, 0, message.Length - inc * 64);
            // Производим дополнение сообщения до длины 512 бит
            ...
            h = G_n(N_0, h, N);
            h = G_n(N_0, h, Sigma);
            // Для хэша с длиной в 512 бит возвращаем h в качестве результата
            if (outLen == 512)
                return h;
            // Для хэша длиной 256 бит возвращаем половину результата
            else
            {
